...
/// This internal class is core of DatabaseManager as it configures and accesses Core Data Stack.
open class Stack {
    ...
    public class var defaultModel: NSManagedObjectModel {
        //let messageKitBundle = Bundle(identifier: "com.app.CrewP2PFramework")
        let modelURL = Bundle.module.url(forResource: "Database_P2P", withExtension: "momd")!
    ...
    func executeInBackground<T: NSManagedObject>(fetchRequest request: NSFetchRequest<T>,
                                               in context: NSManagedObjectContext,completion: @escaping (([T]) -> ())) {
        
        DispatchQueue.global(qos: .background).async {
            var fetchedObjects = [T]()
            context.perform {
                print("Is current thread main thread?: \(Thread.current.isMainThread)")
                if let objects = try? context.fetch(request) {
                    fetchedObjects = objects
        ...
        DispatchQueue.global(qos: .background).async {
            context.perform {
                if context.hasChanges {
                    do {
                        try context.save()
                        DatabaseViewerViewModel.instance.setPublisher()
                    } catch {
    ...
    class func refreshObjects(with objectIDs: [NSManagedObjectID], mergeChanges: Bool,
                              in context: NSManagedObjectContext = DatabaseManager.Context.default) {
